...
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@100..900&display=swap');
    ...
    .count_box {font-size: 2.8rem; color: #fff; position: absolute; top: 22.5%; left: 66.2%; transform: translateX(-50%); min-width: 9%;}
    .count {display: block; color: #ffde00; font-size: 390% !important; font-family: 'GmarketSans'; font-weight: 500; line-height: 1; text-align: right;}
    ...
    #page_landing_c .wrap_form {padding: 0; border: 0;  background: url('//static.savemkt.com/event/v_${eventSeq}/bg.jpg') no-repeat center top / 100% 110%;}
    ...
    .top-img {width: 93.236%; margin: 0 auto; padding-bottom: 5%;}
    ...
    #page_landing_c .wrap_curd {padding: 5.464% 2.927%; background: transparent;}
    #page_landing_c .wrap_curd .q_select {width: 100%; font-size: 150%; display: flex; align-items: center; justify-content: flex-end;}
    #page_landing_c .wrap_form .description {width: 100%; margin-top: 5%;}
    #page_landing_c .wrap_form .description strong {color: #000; font-weight: 700;}
    #page_landing_c .wrap_form .description .ad_txt {font-family: "Noto Sans JP";}
    ...
    #page_landing_c .wrap_form .agBox {position: relative; display: flex; align-items: center; justify-content: center; font-family: "Noto Sans JP";}
    #page_landing_c .wrap_form .agBox .btn-agreement {font-weight: 700; font-family: "Noto Sans JP"; font-size: 130%;}
    ...
    /* 왼쪽 질문 텍스트 영역 너비 고정 */
    #page_landing_c .wrap_form .legend { flex: 0 0 30%; /* 질문 영역 너비를 30%로 고정 */ font-size: 1.5rem; /* 폰트 크기 조정 */ font-family: "Noto Sans JP"; font-weight: 700; color: #000; text-align: left;}
    ...
    #page_landing_c .wrap_form .legend {font-family: "Noto Sans JP", sans-serif; font-size: 280%; min-width: 24%; font-weight: 700; line-height: 1; letter-spacing: -0.05em; color: #0a151e;}
    ...
    .wrap_form .agBox a {font-weight: 700; margin: 0; font-family: "Noto Sans JP";}
...
@media screen and (max-width: 768px){
    ...
    .count_box {top: 27%; left: 66.5%;}
    .count {font-size: 190% !important;}

    #page_landing_c .wrap_form {background-size: 100% 105%;}
            ...
            <div class="wrap_form">
                <form class="wrap_curd" id="form-1" method="POST" accept-charset="utf-8">
                        ...
                        <div class="question_container">
                            ...
                            <div class="sub-txt"><img src="//static.savemkt.com/event/v_${eventSeq}/txt_01.png"></div>
                            <div class="question_box">
                                <div class="user_name">
                                    <span class="legend">お名前</span>
                                    <input type="text" name="name" id="name" class="inp" placeholder="お名前をご入力ください" required>
                                </div>

                                <div class="user_phone">
                                    <span class="legend">ご連絡先</span>
                                    <input type="tel" name="phone" id="phone" class="inp" placeholder="「-」なしで入力してください" value="090" maxlength="11" required>
                                </div>

                                <div class="user_id">
                                    <span class="legend">LINE ID</span>
                                    <input type="text" name="tadd1" class="inp" placeholder="IDを入力してください" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="description">
                            <div class="ad_txt"><div class="img-area"><img src="//static.savemkt.com/event/v_${eventSeq}/txt_02.png"></div></div>
                        </div>
...
<script>
    ...
    // 신청 인원 카운트다운
    $(document).ready(function() {
        let count = 20; // 초기 인원 수
        const intervalTime = 5000; // 2초마다 업데이트
	...
	document.addEventListener('DOMContentLoaded', function() {
	    ...
	    // 페이지 로드 시 기본값 설정
	    phoneInput.value = '090';

	    // 입력값이 사라지면 다시 '090' 넣기
	    phoneInput.addEventListener('input', function() {
	        if (!phoneInput.value.startsWith('090')) {
	            phoneInput.value = '090';
	        }
	    });
	});
	...
	function fnForm(formId){
		...
		// LINE ID(tadd1) → add1 hidden 으로 복사
		...
		// 이 페이지 전용 검증 (form-event.js validateForm 의 "010" 체크를 우회)
		const nameInput  = procForm.querySelector('[name="name"]');
		const phoneInput = procForm.querySelector('[name="phone"]');
		const add1Input  = procForm.querySelector('[name="add1"]');
		...
		if (typeof containsAbuseWords === 'function' && containsAbuseWords(nameInput.value)) {
			alert("入力内容に不適切な単語が含まれています。");
			nameInput.focus();
			return;
		}
		if (nameInput.value.length < 2) {
			alert("正しいお名前をご入力ください。");
			nameInput.focus();
			return;
		}

		// 전화번호 체크 (일본: 090 시작, 11자리)
		...
		if (!/^\d+$/.test(phoneInput.value)) {
			alert("ご連絡先は数字のみご入力ください。");
			phoneInput.focus();
			return;
		}
		if (!phoneInput.value.startsWith("090")) {
			alert("ご連絡先は090から始まる必要があります。");
			phoneInput.focus();
			return;
		}
		if (phoneInput.value.length !== 11) {
			alert("ご連絡先は11桁の数字でご入力ください。(例: 09012345678)");
			phoneInput.focus();
			return;
		}

		// LINE ID(add1) 빈값 체크
		if (!add1Input || !add1Input.value.trim()) {
			alert("LINE IDをご入力ください。");
			if (selectedRadio1) selectedRadio1.focus();
			return;
		}

		// 검증 통과 → 실제 제출 (form-event.js 의 fnFormProc 재사용)
		if (typeof fnFormProc === 'function') {
			fnFormProc(procForm);
		} else {
			alert("送信処理を初期化できませんでした。ページを再読み込みしてください。");
		}
	}
</script>