            ...
            <section class="page" data-value="4">
                <div class="sectionInner">
                    <div class="question_box">
                        ...
                        <input type="hidden" name="branch" id="branch" class="inp" value="부산점" required="">
                    </div>
                    <div class="agBox">
...
<script>
	...
	function processPageTransition(pageNum) {
		let pageNumNext = pageNum +1;
		$('[data-value="'+pageNum+'"]').fadeOut(function() {
		$('[data-value="'+pageNumNext+ '"]').fadeIn();
		$('.gage-area img').attr('src', '//static.resta.co.kr/event/v_'+${resVo.eventSeq}+'/gage_0'+pageNumNext+'.png'); // 게이지 바
		});
	}
	
	function goToNextPage(pageNum) {
		let nm = 'tadd'+pageNum;
		let checkedInput = document.querySelector('input[name="'+nm+'"]:checked');
		switch (pageNum) {
		// case 4: // 4페이지 (이름)
