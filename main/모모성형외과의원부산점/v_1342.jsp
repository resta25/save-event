...
<style>
	* {font-family: 'Gmarket Sans';}
	...
	input[type="submit"] {
	    ...
	    outline: none;
	    padding: 5% 0;
		background: url("//static.resta.co.kr/event/v_${eventSeq}/btn_newSb_04.png") no-repeat center center / 100% 100%;
	}
		...
		<div class="form">
                ...
                <div class="poster_01">
                    ...
                    <div class="agreement">
                        ...
                        <ol>
                            ...
                            <li class="desc">*상기 동의를 거부할 권리가 있으나, 수집 및 이용에 동의하지 않을 경우 <?=$this->landing['object']?> 상담 및 이벤트 참여가 불가능합니다.</li>
                        </ol>
                    </div>
                </div>
			...
			<section class="page" id="page-2">
                ...
                <div class="question_box">
					<div class="q_select">
						<label><input type="radio" name="add1" id="add1" value="이마/헤어라인">이마/헤어라인</label>
						<label><input type="radio" name="add1" id="add1" value="정수리/가르마">정수리/가르마</label>
						<label><input type="radio" name="add1" id="add1" value="기타">여러부위</label>
						<label><input type="radio" name="add1" id="add1" value="기타">기타</label>
					</div>
				</div>
			...
			<section class="page" id="page-4">
                ...
                <div class="form-box poster_11">
                    <div class="formGroup">
                        ...
                        <input type="text" name="name" id="name" class="inp" value="" autocomplete="off" placeholder="성함">
                        </div>
                        ...
                        <input type="tel" name="phone" id="phone" class="inp" value="" autocomplete="off" maxlength="11" placeholder="전화번호 ('-'제외)">
                        </div>
                        <input type="hidden" name="branch" id="branch" required value="seomyeon"/>
...
<script>
	...
	let isAnimating = false; // 애니메이션 상태 플래그
	    ...
	    //설문 클릭 시 클래스on 추가
	    $('input[name*="add"], input[name*="age"]').bind('click', function() {
	    ...
	    function getTotalCommentCount(){
	        ...
	        $('.subscribe').each(function(idx,obj) {
	            var ID = $('.content', obj).last().data('id');
	            var limit = $(obj).data('limit');
	            if(typeof ID == 'undefined') ID = '';
	            if(!limit) limit = 999;
	            $.ajax({
	                type: "POST",
	                url: "./<?php echo $this->no;?>/ajaxproc",
	                ...
	                success: function(result){
	                    if(result.result) {
	                        ...
	                        updateContentCount(data); // 콘텐츠 수 업데이트 함수 호출
	    ...
	    // Comment Start
	    function getComment() { //해당 달 댓글 리스트만 보이게
	        // var today = new Date();   
	                    ...
	                    if(result.result) {
	                        ...
	                        for(v in data) {
	                            var html  = '<div class="content" data-id="'+ data[v].seq +'">';
	                                ...
	                                html += '	<div class="date">'+ data[v].reg_date +'</div>';
	                                html += '</div>';
	                                
	                            dateNum = (data[v].reg_date.substr(0, 2));
	                            // if(month == dateNum){ 
	                                $(obj).append(html);
	                            // }							
	                        }
	                        ...
	                        $(obj).append('<div class="none">데이터가 없습니다.</div>');
	                        $('.none', obj).delay(3000).fadeOut();
	                    }
	                }
	            });
	        });
	    ...
	    // 댓글 수 업데이트 함수
	    function updateContentCount(data) {
	        var displayCount = data.length % 1000; // 999를 초과하면 0으로 리셋 후 다시 카운팅
	        $('#eventCnt').text(`${displayCount}명`); // #remaining-num에 업데이트
	    }
	...
	function fnForm(formId){
		...
		let age = procForm.querySelector(`input[name="age"]:checked`).value;
		procForm.querySelector("input[name='age']").value = age;
